...
// DecToInt converts an ascii encoded decimal integer to an int.
func DecToInt(slice []byte) int {
	num := 0
	length := len(slice)
...
// HexToInt converts an ascii encoded hexadecimal integer to an int.
func HexToInt(slice []byte) int {
	num := 0
	length := len(slice)
