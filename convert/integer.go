...
// DecToInt converts an ascii encoded decimal integer to an int64.
func DecToInt(slice []byte) int64 {
	num := int64(0)
	length := len(slice)
...
// HexToInt converts an ascii encoded hexadecimal integer to an int64.
func HexToInt(slice []byte) int64 {
	num := int64(0)
	length := len(slice)
